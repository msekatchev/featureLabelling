...
def obtain_reprojected_points(features):
    ...
    #print("seed_feature_locations = ", seed_feature_locations)
...
####################
        ...
        for item in feature_reprojected_points[filename_no_extension]:
            if(closestItem[:-3] == item[:-3]):
                ...
                #print(all_bolt_locations[item])
                #reprojected_points_3D = np.append(reprojected_points_3D, (all_bolt_locations[item][0],all_bolt_locations[item][1],all_bolt_locations[item][2]))
                reprojected_points_3D.append((all_bolt_locations[item][0],all_bolt_locations[item][1],all_bolt_locations[item][2]))
        ## Determining the search radius
        reprojected_points_3D_array = np.array(reprojected_points_3D)
        
        #print(reprojected_points_3D)
        print(reprojected_points_3D_array)


        for item in pmt_reprojected_points[filename_no_extension]:
            if(closestItem[:-3] == item[:-3]):
                predicted_dynode_point = all_pmt_locations[item]


        print(predicted_dynode_point[0])
        ...
        point, normal = sk.fit_plane(reprojected_points_3D_array)
        print("point=", point,"normal=", normal)
        camera_matrix = build_camera_matrix(focal_length, principle_point)
        distortion = build_distortion_array(radial_distortion, tangential_distortion)
        reprojected_bolt_centre = cv2.projectPoints(point, rotation_vector, translation_vector,camera_matrix, distortion)[0].reshape((1, 2))
        print("reprojected point:", reprojected_bolt_centre)
...
#        x0 = np.concatenate((camera_rotations.flatten(),camera_translations.flatten(),reprojected_points_3D_array.flatten()))
        print("Here2")
#        res = opt.least_squares(fit_errors, x0, verbose=2, method='lm', xtol=1e-6)
        print("Here3")
...
#        print(reco_locations)
        ...
        bolt_array = np.stack(list(reprojected_points_3D_array))

        ax.scatter(bolt_array[:,0], bolt_array[:,1], bolt_array[:,2], marker='.', label="predicted bolt points")
        ax.scatter(point[0],point[1],point[2], marker='^', label="average bolt location")
        ax.scatter(predicted_dynode_point[0],predicted_dynode_point[1],predicted_dynode_point[2], marker = '*', label = "predicted dynode point")


#        X,Y,Z = [[point[i]-normal[i]*100, point[i]+normal[i]*100] for i in range(3)]
#        ax.plot(X,Y,Z,color='orange')

        X,Y,Z = [[predicted_dynode_point[i]-normal[i]*100, predicted_dynode_point[i]+normal[i]*1] for i in range(3)]
...
#        print("There are",len(ordered_input_bolts), "points to look at.")
        ...
        bolt_angle_centre_x = inputPoints[0]
        bolt_angle_centre_y = inputPoints[1]
        bolt_angle_centre_x = reprojected_bolt_centre[0][0]
        bolt_angle_centre_y = reprojected_bolt_centre[0][1]
        ...
        cv2.circle(img,(int(float(bolt_angle_centre_x)),int(float(bolt_angle_centre_y))),5,(255,255,255),-1)
        for bolt_no in range(len(ordered_input_bolts)) :
            #print("--------------------------------")
            #print("Looking at bolt index: ",bolt_no)                           

            currentLineX = float(ordered_input_bolts[bolt_no][0])-float(bolt_angle_centre_x)
            currentLineY = float(ordered_input_bolts[bolt_no][1])-float(bolt_angle_centre_y)
...
if(labellingStats == True):
    incorrectlyMatchedFeatures = comparingLabelling.compare(inputFileType, outputTextFilename, "045-manualLabelling.txt")
