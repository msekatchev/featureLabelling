...
outputTextFilename = os.path.join(filename_no_extension+"-manualLabelling - distance to reprojected bolts.txt")
...
######################Ordering the identified input bolts##########################
        ...
        if(len(nodes) != 0):
            for i in np.linspace(0, 2*np.pi, num=60) : 
                ...
                #print("search_x=",search_x,"search_y=",search_y,"nodes=",nodes)
...
#########################Saving the bolts ##########################################
        ...
        #print("reprojected_points=",reprojected_points)
        #print("ordered_input_bolts=",ordered_input_bolts)
        for j in ordered_input_bolts :
            bolt_no += 1
            #print(bolt_no)
            ...
            outputFile.write("%s\t%s-%s\t%s\t%s\t%s\n" %(filename_no_extension,closestItem[:-3],printBoltNumber, str(j[0]), str(j[1]),initials))
            ...
            lengthx = j[0]-int(inputPoints[0])
            lengthy = j[1]-int(inputPoints[1])
            ...
            #bolts 2 through 14 (on the right side of the circle/the 1st and 4th quadrant) 
                ...
                textx = int(j[0]-(30)*np.cos(angle))
                texty = int(j[1]-(30)*np.sin(angle))
            #bolt 13
            ...
            cv2.line(img, (j[0],j[1]), (textx,texty), (0,0,177), thickness=2, lineType=8, shift=0)

            cv2.line(img, (j[0],j[1]), (textx,texty), (0,177,177), thickness=2, lineType=8, shift=0)
