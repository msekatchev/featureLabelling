...
outputTextFilename = os.path.join(filename_no_extension+"-correctedTapendra.txt")
...
for inputPoints in input_feature_locations:
    ...
    if closestItemPMT == True:
        ...
        for item in source_feature_locations:
            if(PMTID == item[2][:-3]):
                source_bolts.append((int(item[0]),int(item[1]),item[2]))
        #print(len(bolts))
        counter = 0
        average_distance = 0
        #print("bolts=",bolts)
        #print("source_bolts=", source_bolts)
        ...
        for item in bolts:
            ...
            outputFile.write("%s\t%s%s\t%d\t%d\t%s\n" %(filename_no_extension,PMTID,item[2][-2:],int(inputPoints[0]),int(inputPoints[1]),initials))
            cv2.circle(img,(int(inputPoints[0]),int(inputPoints[1])),6,(255,0,0),-1)

            minDistance = 1000
            for source_item in source_bolts:
            #print(bolt_no)

                distance = math.sqrt(     ( int(item[0])-int(source_item[0]) )**2 + ( int(item[1])-int(source_item[1]) )**2)
                if distance < minDistance:
                    minDistance = distance
                    closest_source_item = source_item


            if closest_source_item[2][-2:] != item[2][-2:]:
                cv2.circle(img,(int(float(item[0])),int(float(item[1]))),5,(255,255,255),1)
                text = closest_source_item[2][-2:]
                text = text.lstrip("0")
                cv2.putText(img, f'{text}', (int(float(item[0])),int(float(item[1]))), cv2.FONT_HERSHEY_PLAIN, 1, (255,170,0), 1)
