...
outputTextFilename = os.path.join(filename_no_extension+"-OUTPUT.txt")
...
for inputPoints in input_feature_locations:
    ...
    if closestItemPMT == True:
        ...
        for item in feature_reprojected_points[filename_no_extension]:
            ...
            if(closestItem[:-3]==item[:-3] and item[-3:]=="-01"):
#                print("ITEM",item)
                firstReprojectedBoltX = int(feature_reprojected_points[filename_no_extension][item][0])
                firstReprojectedBoltY = int(feature_reprojected_points[filename_no_extension][item][1])
...
#            print("Looking at bolt index: ",bolt_no)
            ...
            firstReprojectedBoltLineX = firstReprojectedBoltX - int(inputPoints[0])
            firstReprojectedBoltLineY = firstReprojectedBoltY - int(inputPoints[1])

            firstBoltLineLength = math.sqrt(     (firstReprojectedBoltLineX)**2 +  (firstReprojectedBoltLineY)**2)
            ...
            #print(firstBoltLineLength, currentLineLength)
            ...
            if(currentLineY == 0):
                ...
                angle = math.degrees(math.acos(  ( (firstReprojectedBoltLineX*currentLineX)+(firstReprojectedBoltLineY*currentLineY) ) / ( float(firstBoltLineLength)*float(currentLineLength) )  )   )
#                print("Initial Angle is:", angle)
...
#############################################################################################
        ...
        continue
        #cv2.line(img, (int(inputPoints[0]),int(inputPoints[1])), (int(feature_reprojected_points[filename_no_extension][closestItem][0])
        #      ,int(feature_reprojected_points[filename_no_extension][closestItem][1])), (0,0,255), thickness=3, lineType=8, shift=0)
